...
现在每一个 AI 浏览器 Agent 都撞在同样三堵墙上：

**第一堵墙：身份。** 你启动一个无头浏览器。网站看到的是一个全新的访客——没有 cookie，没有登录，没有历史记录。你是陌生人。Cloudflare 让你证明自己是人类。你证明不了。

**第二堵墙：检测。** 你加了 stealth 插件，伪造 User-Agent，随机化 Canvas 指纹。有用——直到网站更新检测逻辑。你再更新伪装。它再更新检测。一场没有赢家的军备竞赛。

**第三堵墙：感知。** 你给 AI 一张截图（贵、慢）或者原始 DOM（5 万个 token 的噪音）。AI 点错了按钮，在错误的输入框里打字，漏掉了弹窗。
